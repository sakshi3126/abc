  ...
  def facebook
    if params[:facebook_access_token]
      ...
      if user
        ...
        if user.save
          json_response('Login Facebook Successful', true, { user: user }, :ok)
        else
          json.response(user.errors, false, {}, :unprocessable_entity)
        end
      end
    else
      json_response("Missing facebook access toke", false, {}, :unprocessable_entity)
    end
